...
void ui::tool::rig_interaction::activate(canvas::manager& canv_mgr) {
    drag_ = {};
}

void ui::tool::rig_interaction::keyReleaseEvent(canvas::scene& c, QKeyEvent* event) {}

void ui::tool::rig_interaction::mousePressEvent(canvas::scene& canv, QGraphicsSceneMouseEvent* event) {
    click_pt_ = event->scenePos();
}

bool  ui::tool::rig_interaction::is_dragging() const {
    return drag_.has_value();
}
std::optional<ui::tool::rubber_band_type> ui::tool::rig_interaction::kind_of_rubber_band(
...
std::optional<ui::tool::drag_state> ui::tool::rig_interaction::create_drag_state(
...
void ui::tool::rig_interaction::mouseReleaseEvent(canvas::scene& canv, QGraphicsSceneMouseEvent* event) {
...
void ui::tool::rig_interaction::handle_click(
...
void ui::tool::rig_interaction::do_rotation_complete(canvas::scene& canv, const rotation_state& ri) {
    if (authoring_animation()) {
        if (animation_authoring_ && animation_authoring_->complete) {
            animation_authoring_->complete(authored_rotation_for(ri));
        }
        else {
...
void ui::tool::rig_interaction::handle_select_drag(canvas::scene& canv, QRectF rect, bool shift_down, bool ctrl_down) {
...
ui::tool::rig_interaction::authored_action ui::tool::rig_interaction::authored_rotation_for(const rotation_state& state) const {
...
std::optional<ui::tool::rig_interaction::authored_action> ui::tool::rig_interaction::authored_translation_for(const translation_state& state) const {
