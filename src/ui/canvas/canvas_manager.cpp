...
void ui::canvas::canvas_manager::disconnect_current_tab_signal() {
    disconnect(current_tab_conn_);
}

ui::canvas::canvas_manager::canvas_manager(input_handler& inp_handler) :
...
void ui::canvas::canvas_manager::init(mdl::project& proj) {
...
void ui::canvas::canvas_manager::clear() {
    active_canv_ = nullptr;
    while (count() > 0) {
...
void ui::canvas::canvas_manager::add_tab(const std::string& name) {
...
void ui::canvas::canvas_manager::add_or_delete_tab(const std::string& name, bool should_add) {
    if (should_add) {
        add_tab(name);
    }
    else {
...
void ui::canvas::canvas_manager::prepare_to_add_bone(sm::node& u, sm::node& v) {
...
void ui::canvas::canvas_manager::add_new_bone(sm::bone& bone) {
    auto& canv = active_canvas();
...
void ui::canvas::canvas_manager::add_new_skeleton(const std::string& canvas, sm::skel_ref skel_ref) {
...
QGraphicsView& ui::canvas::canvas_manager::active_view() const {
...
std::vector<std::string> ui::canvas::canvas_manager::tab_names() const {
...
void ui::canvas::canvas_manager::set_contents(mdl::project& model) {
...
void ui::canvas::canvas_manager::set_contents_of_canvas(mdl::project& model, const std::string& canvas) {
    auto* canv = canvas_from_name(canvas);
    if (!canv) {
        return;
    }
...
void ui::canvas::canvas_manager::clear_canvas(const std::string& canv)
{
    canvas_from_name(canv)->clear();
}

void ui::canvas::canvas_manager::set_drag_mode(drag_mode dm) {
    drag_mode_ = dm;
